                ...
                {TASK_DEPTH_OPTIONS.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="max-w-full break-words text-xs text-muted-foreground">{t(`repositoryChatSheet.${option.key}-desc`)}</span>
                    </DropdownMenuItem>
                  );
                })}
              </DropdownMenuContent>
            </DropdownMenu>
            {!isSending && lastMessage?.role === 'assistant' && (lastMessage.status === 'error' || lastMessage.status === 'aborted') && <Button type="button" variant="ghost" size="sm" className="h-11 px-2 sm:h-7" onClick={() => void retry()}><RotateCcw className="mr-1 h-3.5 w-3.5" aria-hidden="true" />{t('repositoryChatSheet.retry')}</Button>}
          </div>
        </form>
      </SheetContent>
    </Sheet>
