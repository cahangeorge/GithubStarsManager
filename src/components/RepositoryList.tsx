    ...
    <div className="space-y-5">
      ...
      <div className="ui-toolbar flex min-w-0 flex-col flex-wrap gap-3 p-3 sm:flex-row sm:items-center sm:justify-between sm:gap-0 sm:p-4">
          ...
          {!isLoading && (
            <div className="flex min-w-0 flex-col flex-wrap gap-2 sm:flex-row sm:items-center sm:gap-3">
              ...
              <RadioGroup aria-labelledby="repository-display-content-label" value={showAISummary ? 'ai' : 'original'} onValueChange={(value) => { if (value === 'ai' && !hasAnalyzedRepos) return; setShowAISummary(value === 'ai'); }} className="flex items-center space-x-3 sm:space-x-4">
                <label onClick={() => { if (hasAnalyzedRepos) setShowAISummary(true); }} className={`flex min-h-11 items-center space-x-1.5 sm:min-h-0 sm:space-x-2 ${hasAnalyzedRepos ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`} title={hasAnalyzedRepos ? t('repositoryList.show-ai-generated-analysis-summary') : t('repositoryList.no-ai-analysis-content-available')}>
                ...
                <label onClick={() => setShowAISummary(false)} className="flex min-h-11 cursor-pointer items-center space-x-1.5 sm:min-h-0 sm:space-x-2" title={t('repositoryList.show-repository-original-description')}>
        ...
        <div className={`ml-auto flex min-w-0 w-full flex-col items-end gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3 ${disableCardAnimations ? 'repository-list-syncing' : ''}`}>
          ...
          {!isLoading && (
            <div className="flex min-w-0 shrink-0 items-center gap-1 rounded-lg border border-border bg-muted p-0.5 dark:border-border dark:bg-muted/40" role="group" aria-label={t('repositoryList.repository-layout')}>
              <Button
                type="button"
                variant="ghost"
                size="icon"
